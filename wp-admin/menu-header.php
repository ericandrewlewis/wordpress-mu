...
function _wp_menu_output( $menu, $submenu, $submenu_as_parent = true ) {
	...
	foreach ( $menu as $key => $item ) {
		...
		if ( false !== strpos($class, 'wp-menu-separator') ) {
			...
			if ( ( ('index.php' != $submenu[$item[2]][0][2]) && ( file_exists(WP_PLUGIN_DIR . "/{$submenu[$item[2]][0][2]}") || file_exists( WPMU_PLUGIN_DIR . "/{$submenu[$item[2]][0][2]}" ) ) ) || !empty($menu_hook) ) {
			...
			if ( ('index.php' != $item[2]) && ( file_exists(WP_PLUGIN_DIR . "/{$item[2]}") || file_exists(WPMU_PLUGIN_DIR . "/{$item[2]}") ) || !empty($menu_hook) ) {
				$admin_is_parent = true;
		...
		if ( !empty($submenu[$item[2]]) ) {
			...
			foreach ( $submenu[$item[2]] as $sub_key => $sub_item ) {
				...
				if ( ( ('index.php' != $sub_item[2]) && ( file_exists(WP_PLUGIN_DIR . "/{$sub_item[2]}") || file_exists( WPMU_PLUGIN_DIR . "/{$sub_item[2]}") ) ) || ! empty($menu_hook) ) {
					// If admin.php is the current page or if the parent exists as a file in the plugins or admin dir
					$parent_exists = (!$admin_is_parent && file_exists(WP_PLUGIN_DIR . "/{$item[2]}") && !is_dir(WP_PLUGIN_DIR . "/{$item[2]}") || ( file_exists( WPMU_PLUGIN_DIR . "/{$item[2]}" ) && ! is_dir( WPMU_PLUGIN_DIR . "/{$item[2]}" ) ) ) || file_exists($item[2]);
					if ( $parent_exists )
